...
Public Sub CATIA_VISUAL_BOM_EXPORTER()
    ...
    If Not ExportBomWithBillOfMaterialPrintXls(gExcelPath) Then
...
Function BuildBomExcelPath(selectedFolder, rootPartNumber)
    ...
    BuildBomExcelPath = JoinPath(selectedFolder, baseName & "_VISUAL_BOM_EXPORT.xls")
End Function
...
Function ExportBomWithBillOfMaterialPrintXls(bomExcelPath)
    On Error Resume Next
    ExportBomWithBillOfMaterialPrintXls = False
    ...
    WriteDebugPhase "BOM_FORMAT_SET", 0, "", "", "", "Setting CATIA BOM secondary format."
    Err.Clear
    assemblyConvertor1.SetSecondaryFormat arrayOfVariantOfBSTR1
    If Err.Number <> 0 Then
        AbortWithMessage "CATIA SetSecondaryFormat nije uspeo: " & Err.Description, "BOM_FORMAT_SET", 0, "", "", bomExcelPath
        Err.Clear
        Exit Function
    End If
    ...
    If Err.Number <> 0 Then
        AbortWithMessage "CATIA BillOfMaterial.Print XLS nije uspeo: " & Err.Description, "CATIA_BOM_PRINT_XLS_START", 0, "", "", bomExcelPath
        Err.Clear
        Exit Function
    End If
    ...
    ExportBomWithBillOfMaterialPrintXls = True
    Err.Clear
End Function
...
Sub WriteExportLog(rowIndex, rawPartNumber, normalizedPartNumber, statusText, phase, messageText, imagePath, thumbPath, sourcePath)
    ...
    If statusText = "SOURCE_FILE_NOT_FOUND" Or statusText = "UNSUPPORTED_SOURCE_FILE" Or statusText = "STANDALONE_OPEN_FAILED" Or statusText = "STANDALONE_CAPTURE_FAILED" Or statusText = "THUMBNAIL_FAILED" Or statusText = "TIMEOUT" Or statusText = "ERROR" Then gErrorCount = gErrorCount + 1
    Err.Clear
End Sub
